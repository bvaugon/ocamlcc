...
/* OCamlCC: obj_counter -> extern_obj_counter (conflicts with intern.c) */

static uintnat extern_obj_counter;  /* Number of objects emitted so far */
...
static void extern_record_location(value obj)
{
  ...
  if (extern_trail_cur == extern_trail_limit) {
    struct trail_block * new_block = (struct trail_block *)
      malloc(sizeof(struct trail_block));
  ...
  Field(obj, 0) = (value) extern_obj_counter;
  extern_obj_counter++;
}
...
static void init_extern_output(void)
{
  ...
  extern_output_first = (struct output_block *)
    malloc(sizeof(struct output_block));
 ...
 tailcall:
  ...
  if (Is_in_value_area(v)) {
    ...
    if (Color_hd(hd) == Caml_blue) {
      uintnat d = extern_obj_counter - (uintnat) Field(v, 0);
      if (d < 0x100) {
...
CAMLexport void caml_serialize_block_2(void * data, intnat len)
{
...
#ifndef ARCH_BIG_ENDIAN
  {
    unsigned char * p;
    char * q;
    for (p = (unsigned char *) data, q = extern_ptr;
         len > 0;
         len--, p += 2, q += 2)
      Reverse_16(q, p);
    extern_ptr = q;
