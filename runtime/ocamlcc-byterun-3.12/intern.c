...
/* OCamlCC: obj_counter -> intern_obj_counter (conflicts with extern.c) */

static asize_t intern_obj_counter;
  ...
  if (code >= PREFIX_SMALL_INT) {
    ...
    read_block:
      if (size == 0) {
        ...
        *dest = v;
        if (intern_obj_table != NULL) intern_obj_table[intern_obj_counter++] = v;
    ...
    read_string:
      ...
      *intern_dest = Make_header(size, String_tag, intern_color);
      ...
      read_shared:
        Assert (ofs > 0);
        Assert (ofs <= intern_obj_counter);
        Assert (intern_obj_table != NULL);
        v = intern_obj_table[intern_obj_counter - ofs];
        break;
      case CODE_SHARED16:
        ofs = read16u();
        goto read_shared;
...
CAMLexport value caml_input_value_from_block(char * data, uintnat len)
{
  uint32 magic;
  mlsize_t block_len;
  value obj;
...
CAMLexport void caml_deserialize_block_2(void * data, intnat len)
{
  ...
  unsigned char * p, * q;
  for (p = intern_src, q = (unsigned char *) data;
       len > 0;
       len--, p += 2, q += 2)
    Reverse_16(q, p);
  intern_src = p;
