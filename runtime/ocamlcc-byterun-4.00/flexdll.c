...
/* OCamlCC: intnat and uintnat already defined in config.h */
/*
typedef long intnat;
...
static void dump_reloctbl(reloctbl *tbl) {
  ...
  for (ptr = tbl->entries; ptr->kind; ptr++)
    /* OCamlCC: %04lx -> %04x */
    printf(" %p (kind:%04x) (now:%p)  %s\n",
           ptr->addr,
           ptr->kind,
    /* OCamlCC: %p -> (void*) cast */
           (void*) *((uintnat*) ptr->addr),
           ptr->name
           );
}
...
/* OCamlCC: unextern static_symtable */
symtbl static_symtable;
...
#endif
#ifdef __MINGW32__ /* OCamlCC: rename constant MINGW -> __MINGW32__ */
  {
