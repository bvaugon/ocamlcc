...
void caml_load_code(int fd, asize_t len)
{
  ...
  if ((asize_t) read(fd, (char *) caml_start_code, caml_code_size)
      != caml_code_size)
  ...
  if (caml_debugger_in_use) {
    ...
    for (i = 0; (asize_t) i < len; i++) caml_saved_code[i] = caml_start_code[i];
  }
#ifdef THREADED_CODE
...
/* OCamlCC: rename ATOM -> ATOM_ (collision windef.h) */
void caml_thread_code (code_t code, asize_t len)
{
  ...
  l[PUSHATOM] = l[ATOM_] = l[MAKEBLOCK1] = l[MAKEBLOCK2] =
