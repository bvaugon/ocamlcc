      ...
      | Closure (env_size, ptr) ->
        ...
        if is_read dst_id then (
          ...
          let new_closure_size =
            let count i b = if b <> Unknown then i else i + 1 in
          ...
          if env_size > 0 && is_cell accu_id && ptr_env_usages.(0) <> Unknown
          then
