...
let compute_fun prims dbug funs tc_set fun_id
...
let gen_macroc prims data dbug funs dzeta_code tc_set =
  ...
  let fold_fun_decl _ (fd, _, _, _, _, _) acc =
    let use_env = Body.test_useenv fd in
    let inlinable = Body.test_inlinable funs fd in
  ...
  let fold_fun_def fun_id dzeta acc =
    let fun_def = compute_fun prims dbug funs tc_set fun_id dzeta in
    fun_def :: acc
  in
  let max_arity = Body.compute_maximum_arity funs in
  let fun_decls = List.rev (IMap.fold fold_fun_decl rest []) in
  Options.message ".";
  let main_def = compute_fun prims dbug funs tc_set 0 main in
  Options.message ".";
