...
value ocamlcc_generic_apply(value arg_nb_val, value arg1, value closure) {
  ...
  if (arity_val == arg_nb_val) {
    ...
    long total_passed = nb_already_passed + arity;

    if (total_passed == sub_arity) {
      long i;
      for (i = arg_nb - 2 ; i >= 0 ; i --)
        ocamlcc_global_params[i + nb_already_passed - 1] =
      ...
      Ocamlcc_alloc_small(new_closure, total_passed + 1, Closure_tag,
                          ...
                          *--caml_extern_sp = arg1;
                          for (i = 0 ; i < arg_nb - 2 ; i ++)
                            *--caml_extern_sp = ocamlcc_global_params[i];
                          ,
                          for (i = arg_nb - 3 ; i >= 0 ; i --)
...
#define ocamlcc_dynamic_apply(nargs, curr_fsz, next_fsz, dst, env, arg1, rest...) { \
...
#define ocamlcc_static_apply(nargs, curr_fsz, next_fsz, dst, f, env, arg1, rest...) { \
  ocamlcc_apply_init_stack(curr_fsz, next_fsz);                         \
  ocamlcc_store_args_##nargs(rest);                                     \
...
#define ocamlcc_dynamic_standard_appterm(nargs, curr_fsz, env, arg1, rest...) { \
  caml_extern_sp = sp;                                                  \
  ocamlcc_store_args_##nargs(rest);                                     \
...
#define ocamlcc_static_standard_appterm(f, env, arg1, rest...) {        \
  caml_extern_sp = sp;                                                  \
  ocamlcc_store_args_##nargs(rest);                                     \
  ocamlcc_global_env = env;                                             \
  return f(arg1);                                                       \
...
#define ocamlcc_static_special_appterm(tc_nargs, f, args...)    \
...
#define ocamlcc_static_standard_appterm(f, args...) {   \
  caml_extern_sp = sp;                                  \
  return f(args);                                       \
}
