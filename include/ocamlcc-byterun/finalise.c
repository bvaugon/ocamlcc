...
{
  struct to_do *result = (struct to_do *)
    malloc (sizeof (struct to_do) + size * sizeof (struct final));
...
CAMLprim value caml_final_register (value f, value v)
{
  ...
  if (young >= size){
    if (final_table == NULL){
      uintnat new_size = 30;
      final_table = (struct final *)
        caml_stat_alloc (new_size * sizeof (struct final));
      Assert (old == 0);
      Assert (young == 0);
      size = new_size;
    }else{
