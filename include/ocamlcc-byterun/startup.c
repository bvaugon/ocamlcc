...
int32 caml_seek_optional_section(int fd, struct exec_trailer *trail,
                                 const char *name)
{
  long ofs;
  int i;
...
int32 caml_seek_section(int fd, struct exec_trailer *trail, const char *name)
{
...
/* OCamlCC: unused (caml_main replacement) */
...
CAMLextern intnat ocamlcc_global_data_length;
...
/*   if (cds_file != NULL) { */
/*     caml_cds_file = (char *) caml_stat_alloc(strlen(cds_file) + 1); */
